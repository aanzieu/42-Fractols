...
__global__ void burning_2(unsigned int* a_d, int constw, int consth, double xmin, double xmax, double ymin, double ymax, double zoom, float offx, float offy, float max, int power)
{
	int x = (blockIdx.x * blockDim.x) + threadIdx.x;
	int y = (blockIdx.y * blockDim.y) + threadIdx.y;
	int index = y * constw + x;
	...
	while (zr * zr + zi * zi < 4 && i < max)
	{
		...
		{
			tmpr = pow(zr, 3) - 3 * zr * zi * zi + cr;
			tmpi = 3 * fabs(zr * zr * zi - pow(zi, 3)) + ci;
		}
		if (power == 2)
		{
			tmpr = zr * zr - zi * zi + cr;
			tmpi = fabs(zr * zi) * 2 + ci;
		}
		zr = tmpr;
		zi = tmpi;
		i++;
...
extern "C" void cuda_call_burning(unsigned int* a_h, unsigned int size, t_env *e, int end)
{
	unsigned int *a_d = NULL;
	static dim3 blockDim(16, 16);
	static dim3 gridDim(WIN_WIDTH / blockDim.x, WIN_HEIGTH / blockDim.y);
	...
	e->view.ymax = 1.25;
	cudaMalloc((void**) &a_d, size*sizeof(unsigned int));
	burning_2 <<< gridDim, blockDim >>> (a_d, WIN_WIDTH, WIN_HEIGTH, e->view.xmin, e->view.xmax, e->view.ymin, e->view.ymax, e->plan.zoom, e->plan.offx, e->plan.offy, e->plan.i_max, e->plan.power);
	cudaMemcpy(a_h, a_d, size*sizeof(unsigned int), cudaMemcpyDeviceToHost);
	cudaFree(a_d);
	if(end == 1)
		cudaDeviceReset();
